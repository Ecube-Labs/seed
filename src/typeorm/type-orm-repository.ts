...
export abstract class TypeOrmRepository<T extends Aggregate<T>, ID> extends Repository<T, ID> {
   ...
   * @param options
   */
  async findOneOrFail(id: ID, options?: { lock: { mode: 'pessimistic_read' | 'pessimistic_write' } }): Promise<T> {
   ...
   * @param options
   */
  async findByIds(ids: ID[], options?: { lock: { mode: 'pessimistic_read' | 'pessimistic_write' } }): Promise<T[]> {
